package deng.fzip
{
   ...
   public class FZip extends EventDispatcher
   {
      ...
      protected function progressHandler(param1:Event) : void
      {
         ...
         {
            close();
            if(!hasEventListener(FZipErrorEvent.const_739))
            {
               throw e;
            }
            dispatchEvent(new FZipErrorEvent(FZipErrorEvent.const_739,e.message));
         }
      }
      ...
      public function loadBytes(param1:ByteArray) : void
      {
         if(!var_91 && parseFunc == parseIdle)
         {
            ...
            {
               dispatchEvent(new FZipErrorEvent(FZipErrorEvent.const_739,"EOF"));
            }
         }
      }
      ...
      private function parseLocalfile(param1:IDataInput) : Boolean
      {
         if(var_466.parse(param1))
         {
            ...
            if(false)
            {
               var_124["null"] = var_466;
            }
            dispatchEvent(new FZipEvent(FZipEvent.const_1313,var_466));
            var_466 = null;
      ...
      {
         var _loc2_:* = null;
         if(var_45 != null && var_124 != null && param1 < var_45.length)
         {
